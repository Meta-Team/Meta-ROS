...
#ifndef TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OMNI_WHEEL_CONTROLLER_HPP_
#define TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OMNI_WHEEL_CONTROLLER_HPP_
...
using ControllerStateMsg = omni_wheel_controller::OmniWheelController::ControllerStateMsg;
using ControllerReferenceMsg = omni_wheel_controller::OmniWheelController::ControllerReferenceMsg;
using ControllerModeSrvType = omni_wheel_controller::OmniWheelController::ControllerModeSrvType;
...
{
  FRIEND_TEST(OmniWheelControllerTest, all_parameters_set_configure_success);
  FRIEND_TEST(OmniWheelControllerTest, activate_success);
  FRIEND_TEST(OmniWheelControllerTest, reactivate_success);
  FRIEND_TEST(OmniWheelControllerTest, test_setting_slow_mode_service);
  FRIEND_TEST(OmniWheelControllerTest, test_update_logic_fast);
  FRIEND_TEST(OmniWheelControllerTest, test_update_logic_slow);
  FRIEND_TEST(OmniWheelControllerTest, test_update_logic_chainable_fast);
  FRIEND_TEST(OmniWheelControllerTest, test_update_logic_chainable_slow);

public:
  controller_interface::CallbackReturn on_configure(
    const rclcpp_lifecycle::State & previous_state) override
  {
    auto ret = omni_wheel_controller::OmniWheelController::on_configure(previous_state);
    ...
    const rclcpp_lifecycle::State & previous_state) override
  {
    ...
    return omni_wheel_controller::OmniWheelController::on_activate(previous_state);
  }
...
class OmniWheelControllerFixture : public ::testing::Test
{
public:
  ...
  void SetUp()
  {
    ...
    command_publisher_ = command_publisher_node_->create_publisher<ControllerReferenceMsg>(
      "/test_omni_wheel_controller/commands", rclcpp::SystemDefaultsQoS());
    ...
    slow_control_service_client_ = service_caller_node_->create_client<ControllerModeSrvType>(
      "/test_omni_wheel_controller/set_slow_control_mode");
  }
...
protected:
  void SetUpController(const std::string controller_name = "test_omni_wheel_controller")
  {
  ...
  void subscribe_and_get_messages(ControllerStateMsg & msg)
  {
    ...
    auto subscription = test_subscription_node.create_subscription<ControllerStateMsg>(
      "/test_omni_wheel_controller/state", 10, subs_callback);
...
#endif  // TEMPLATES__ROS2_CONTROL__CONTROLLER__TEST_OMNI_WHEEL_CONTROLLER_HPP_
