...
void DjiMotorNetwork::init_rx_tx() {
    rx_thread_ = std::thread(&DjiMotorNetwork::rx_loop, this);
    tx_thread_ = std::thread(&DjiMotorNetwork::tx_loop, this);
}
...
void DjiMotorNetwork::write(uint32_t joint_id, double effort) {
    ...
    auto current_raw =
        static_cast<int16_t>(effort / motor->get_maximum_current() * 16384);
    ...
    tx_frame->data[2 * ((dji_motor_id - 1) % 4)] = current_raw >> 8;
    tx_frame->data[2 * ((dji_motor_id - 1) % 4) + 1] = current_raw & 0xFF;
}
...
[[noreturn]] void DjiMotorNetwork::tx_loop() {
    while (true) {
        try {
