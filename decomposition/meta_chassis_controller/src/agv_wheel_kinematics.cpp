...
AgvWheelKinematics::AgvWheelKinematics(const agv_wheel_center_x, const agv_wheel_center_y, double agv_wheel_radius){
...
std::tuple<double,double> AgvWheelKinematics::get_output(double curr_pos, double curr_vel, double target_pos, double target_vel){
    ...
    double angle_diff = angles::shortest_angular_distance(curr_poss, target_pos);
    if(curr_vel > 0 && angle_diff < PI / 2 && angle_diff > - PI / 2){       // FIXME: This is not correct here
        return {target_pos, target_vel};
    }else if(curr_vel < 0 && (angle_diff > PI / 2 || angle_diff < - PI / 2)){
        return {target_pos, target_vel};
    }else{
        return {angles::normalize_angle(PI * 2 - target_pos), -target_vel};
    }
}

void AgvWheelKinematics::inverse(std::vector<double> twist, std::vector<double> & curr_pos, std::vector<double> & curr_vel) const {
    // twist: [vx, vy, wz]
    // curr_pos: [x, y, theta]
    // curr_vel: [v, w]
    ...
    double right_forward_angle = atan(right_forward_y, right_forward_x);
    double right_back_angle = atan(right_back_y, right_back_x);
