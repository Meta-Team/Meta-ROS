...
OmniWheelKinematics::OmniWheelKinematics(
    const std::vector<double> &omni_wheel_angles, double omni_wheel_distance,
    double omni_wheel_radius)
    : omni_wheel_angles_(omni_wheel_angles),
      omni_wheel_distance_(omni_wheel_distance),
...
void OmniWheelKinematics::init() {
    motion_mat_ = Eigen::MatrixXd::Zero(
        static_cast<Eigen::Index>(omni_wheel_angles_.size()), 3);
    for (size_t i = 0; i < omni_wheel_angles_.size(); i++) {
        motion_mat_(static_cast<Eigen::Index>(i), 0) =
            cos((omni_wheel_angles_[i] - 90.0) / 180.0 * M_PI);
        motion_mat_(static_cast<Eigen::Index>(i), 1) =
            sin((omni_wheel_angles_[i] - 90.0) / 180.0 * M_PI);
