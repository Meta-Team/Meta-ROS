...
using ControllerReferenceMsg =
    omni_wheel_controller::OmniWheelController::ControllerReferenceMsg;
...
controller_interface::CallbackReturn OmniWheelController::on_init() {
  ...
  try {
    param_listener_ =
        std::make_shared<omni_wheel_controller::ParamListener>(get_node());
...
controller_interface::CallbackReturn OmniWheelController::on_configure(
  ...
  ref_subscriber_ =
      get_node()->create_subscription<ControllerReferenceMsgUnstamped>(
          ...
          std::bind(&OmniWheelController::reference_callback, this,
                    std::placeholders::_1));
  ...
  follow_pid_ = std::make_shared<control_toolbox::PidROS>(get_node(), "follow_pid_gains", true);
...
void OmniWheelController::reference_callback(
...
OmniWheelController::on_export_reference_interfaces() {
...
OmniWheelController::update_reference_from_subscribers() {
  auto current_ref =
      *(input_ref_.readFromRT()); // A shared_ptr must be allocated immediately
                                  // to prevent dangling
...
controller_interface::return_type
OmniWheelController::update_and_write_commands(const rclcpp::Time &time,
...
PLUGINLIB_EXPORT_CLASS(omni_wheel_controller::OmniWheelController,
                       controller_interface::ChainableControllerInterface)
